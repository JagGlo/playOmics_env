...
pred_spread <- predictions %>% 
  select(model_name, .pred_ductal, ID) %>% 
  spread(model_name, .pred_ductal) %>% 
  select(-ID) %>% 
  mutate_all(as.numeric)
```

```{r}
exemplary_dataset <- validation_set %>% 
...
metrics_test_stepwise(selected_models, exemplary_dataset, pred_spread, "miss", "test_mcc", lambda = lambda, vec_of_ensemble_size = c(1, 3, 5, 7, 9, seq(10,50, by = 5)), voting = "soft")
  })
...
ensembling_results %>% 
  ...
  x$predictions
   ...
   bind_rows() %>% 
  select(-metric_for_model_selection, -tested_metric) %>% 
